...
void SQLiteManager::update(std::string key, std::string	value)
{
	this->open();
	// insert data
	sqlite3_stmt* stmt;
	std::string sql = "UPDATE data SET value = '"+value+"'  WHERE key = '"+key+"'";
	//std::string sql = "UPDATE data SET value = '?'  WHERE key ='?' ";
	if (sqlite3_prepare_v2(_db, sql.c_str(), -1, &stmt,
		nullptr) == SQLITE_OK) {
		sqlite3_bind_text(stmt, 2, value.c_str(), -1,
			SQLITE_TRANSIENT);
		if (sqlite3_step(stmt) != SQLITE_DONE) {
			CCLOG("Error in udpate 1, %s",
				sqlite3_errmsg(_db));
		}
	}
	sqlite3_reset(stmt);
